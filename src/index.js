const apiId = "https://api.github.com/repos";
//Syntax GET /repos/:user/:repo

let URL = location.pathname;
let size;

//KB TO MB
function formatBytes(bytes, decimals = 2) {
  if (bytes === 0) return "0 Bytes";

  const k = 1024;
  const dm = decimals < 0 ? 0 : decimals;
  const sizes = ["KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"];

  const i = Math.floor(Math.log(bytes) / Math.log(k));

  return parseFloat((bytes / Math.pow(k, i)).toFixed(dm)) + " " + sizes[i];
}

//data fetching
const FetchSize = async (url) => {
  try {
    const res = await fetch(`${apiId}${url}`);
    const Data = await res.json();
    size = Data.size;
  } catch (error) {
    console.error(error);
  }
};
//injecting to DOM
FetchSize(URL).then(() => {
  const UL = document.querySelector("#repository-container-header > nav > ul");
  const li = `<li  class="d-inline-flex"><svg class="octicon octicon-database" aria-hidden="true" height="16" version="1.1" viewBox="0 0 12 16" width="12"> 
  <path d="M6 15c-3.31 0-6-.9-6-2v-2c0-.17.09-.34.21-.5.67.86 3 1.5 5.79 1.5s5.12-.64 5.79-1.5c.13.16.21.33.21.5v2c0 1.1-2.69 2-6 2zm0-4c-3.31 0-6-.9-6-2V7c0-.11.04-.21.09-.31.03-.06.07-.13.12-.19C.88 7.36 3.21 8 6 8s5.12-.64 5.79-1.5c.05.06.09.13.12.19.05.1.09.21.09.31v2c0 1.1-2.69 2-6 2zm0-4c-3.31 0-6-.9-6-2V3c0-1.1 2.69-2 6-2s6 .9 6 2v2c0 1.1-2.69 2-6 2zm0-5c-2.21 0-4 .45-4 1s1.79 1 4 1 4-.45 4-1-1.79-1-4-1z"></path>
  </svg> &nbsp ${formatBytes(size)}</li>`;
  UL.innerHTML += li;
});
